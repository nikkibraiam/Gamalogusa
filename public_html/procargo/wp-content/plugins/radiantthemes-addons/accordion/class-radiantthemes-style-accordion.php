...
if ( ! class_exists( 'RadiantThemes_Style_Accordion' ) ) {
	...
	class RadiantThemes_Style_Accordion {
					...
					'params'      => array(
						array(
							...
							'value'       => array(
								...
								esc_html__( 'Style Two (Solid Plus-Minus Style)', 'radiantthemes-addons' )     => 'two',
							),
							'std'         => 'one',
							'admin_label' => true,
						),
			...
			vc_map(
				array(
					...
					'params'                  => array(
                        ...
                        array(
                            "type"        => "checkbox",
                            "heading"     => esc_html__( "Active", "radiantthemes-addons" ),
                            "param_name"  => "accordion_item_active",
                            "value"       => false,
                            "description" => esc_html__( "Make this item active or not.", "radiantthemes-addons" ),
                        ),
						array(
							'type'       => 'textarea_html',
							'holder'     => 'div',
		...
		public function radiantthemes_accordion_style_func( $atts, $content = null, $tag ) {
			...
			// ADD CUSTOM CSS.
			$custom_css = $shortcode['accordion_color'] ? '.radiantthemes-accordion.element-one.'.$random_class.' .radiantthemes-accordion-item > .radiantthemes-accordion-item-title > .radiantthemes-accordion-item-title-icon i.main-icon:before{
            	color: '.$shortcode['accordion_color'].' ;
			}
			.radiantthemes-accordion.element-two.'.$random_class.' .radiantthemes-accordion-item.radiantthemes-active > .radiantthemes-accordion-item-title{
            	background-color: '.$shortcode['accordion_color'].' !important;
			}' : '';
		...
		public function radiantthemes_accordion_style_item_func( $atts, $content = null, $tag ) {
			...
			$active = ( 'true' == $shortcode['accordion_item_active'] ) ? 'radiantthemes-active' : '';
