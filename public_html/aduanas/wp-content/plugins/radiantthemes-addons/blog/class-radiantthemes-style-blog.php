...
if ( class_exists( 'WPBakeryShortCode' ) && ! class_exists( 'Radiantthemes_Style_Blog' ) ) {
	...
	class Radiantthemes_Style_Blog extends WPBakeryShortCode {
				...
				array(
					...
					'params'      => array(
						array(
							...
							'value'      => array(
								esc_html__( 'Style One (Classic No Excerpt)', 'radiantthemes-addons' )  => 'one',
							),
							'std'        => 'one',
						),
						array(
		...
		public function radiantthemes_blog_style_func( $atts, $content = null, $tag ) {
			...
			// ADD CUSTOM CSS.
			$custom_css = $shortcode['blog_color'] ? '.blog.element-one.'.$random_class.' .blog-item > .holder > .data > ul.meta-data > li.author{
