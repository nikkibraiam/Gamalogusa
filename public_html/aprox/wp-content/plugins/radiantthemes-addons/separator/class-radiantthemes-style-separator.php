...
if ( class_exists( 'WPBakeryShortCode' ) && ! class_exists( 'Radiantthemes_Style_Separator' ) ) {
	...
	class Radiantthemes_Style_Separator extends WPBakeryShortCode {
			...
			vc_map(
				array(
					...
					'params'      => array(
						...
						array(
							'type'        => 'textfield',
							'heading'     => esc_html__( 'Separator Width (In Pixels) eg. 5px', 'radiantthemes-addons' ),
		...
		public function radiantthemes_separator_style_func( $atts, $content = null, $tag ) {
			...
			// ADD MAIN CSS.
			wp_register_style(
				'radiantthemes_separator_' . $shortcode['radiantthemes_separator_style'],
				plugins_url( 'radiantthemes-addons/separator/css/radiantthemes-separator-element-' . $shortcode['radiantthemes_separator_style'] . '.css' )
			);
			wp_enqueue_style( 'radiantthemes_separator_' . $shortcode['radiantthemes_separator_style'] );
			...
			$output .= '<div class="radiantthemes-separator-block" style="width: ' . esc_attr( $shortcode['radiantthemes_separator_width'] ) . ' ; height: ' . esc_attr( $shortcode['radiantthemes_separator_height'] ) . ' ; background-color: ' . esc_attr( $shortcode['radiantthemes_separator_color'] ) . ' ;"></div>';
			$output .= '</div>';
			$output .= '<!-- radiantthemes-separator -->';
			return $output;
		}
